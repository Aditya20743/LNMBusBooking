...
export const fetchBus = () => async (dispatch) => {
  try {
    dispatch(requestBus());
    const user = auth.currentUser;
    if (user === undefined) throw Error("Error 401: Unauthorized");
...
export const postSpecialBusRequest =
  (user, specialbusrequest) => async (dispatch) => {
...
export const deleteSpecialBusRequest =
  (user, specialbusrequest) => async (dispatch) => {
    dispatch(requestSpecialBusRequest());
    try {
      if (user !== undefined && user.role === "admin") {
        const specialBusRef = doc(
          db,
          "specialBusRequest",
          specialbusrequest.specialBusId
        );
...
export const checkUser = () => (dispatch) => {
  try {
    if (localStorage.getItem("user") === null) {
      ...
      if (user.role === "caretaker") dispatch(fetchOutpass(user));
      if (user.role === "admin") dispatch(fetchSpecialBusRequest(user));
      else {
        dispatch(fetchWallet(user));
        dispatch(fetchTicket(user));
